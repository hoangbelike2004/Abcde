...
public class GridManager : MonoBehaviour
{
    ...
    private void UpdateBorderAmountText(BorderUnit border, int minX, int maxX, int minY, int maxY, [Bridge.Ref] Color themeColor)
    {
    ...
    private void SetBorderBounds(RectTransform rt, [Bridge.Ref] Bounds bounds)
    {
        if (rt == null) return;
    ...
    private void SmoothSetBorderBounds(RectTransform rt, [Bridge.Ref] Bounds bounds)
    {
        if (rt == null) return;
        rt.anchorMin = new Vector2(0.5f, 0.5f);
        rt.anchorMax = new Vector2(0.5f, 0.5f);
